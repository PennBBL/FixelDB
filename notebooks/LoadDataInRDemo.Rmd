...
fetch_fixel_df <- function(fixel_id, variable, connection){
  ...
  tryCatch(
    ...
    # run query
    query_res <- tryCatch(error = function(cnd) cnd, dbGetQuery(con, sprintf("SELECT * FROM %s A INNER JOIN phenotypes B ON A._id = B._id WHERE A.fixel_id = %d", variable, fixel_id)))
